...
async def send_weekly_summary(bot: Bot):
    ...
    """
    logger.info("🚀 Запуск еженедельной задачи: поиск самого активного пользователя...")
    
    # 1. Получаем данные из БД
    activity_data = await history_service.get_weekly_activity()
    
    if not activity_data:
        logger.info("Недостаточно данных для еженедельного отчета. Пропускаем.")
        return
    ...
    logger.info(f"Найден самый активный пользователь недели: {user_mention} (ID: {user_id})")
    ...
    if summary_text:
        try:
            ...
            logger.info(f"Еженедельный 'подкол' для {user_mention} успешно отправлен.")
        except Exception as e:
            logger.error(f"Не удалось отправить еженедельный отчет в чат: {e}")
    else:
        logger.warning("Не удалось сгенерировать текст для еженедельного отчета.")
...
def setup_scheduler(bot: Bot):
    ...
    scheduler.start()
    logger.info("Планировщик еженедельных задач запущен.")